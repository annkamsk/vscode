...
type CSSStyleDeclarationProperty = Exclude<keyof CSSStyleDeclaration, 'length' | 'parentRule' | 'getPropertyPriority' | 'getPropertyValue' | 'item' | 'removeProperty' | 'setProperty'>;

export function createHTMLElement<T extends HTMLElement>(tag: HTMLElementTagName, content?: string, className?: string, style?: Map<CSSStyleDeclarationProperty, string>): T {
	...
	if (style) {
		style.forEach((value, key) => element.style[key] = value);
	}
	return element as T;
}
